...
-- задание з (закомментировал, чтобы не было ошибок)
-- call fillWithDepr();
...
-- задание т
-- view может расссматриваться как синоним в mysql, так как она динамически обновляется
-- доступность представления будет зависеть только от прав пользователя на это представление
-- создадим в этой же бд, но по идее должно быть в другой
create view synonim as (select * from test_table);
